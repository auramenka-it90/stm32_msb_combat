 ...
 ******************************************************************************
 * @file    fpga_control.h
 * @brief   Header for High-Level FPGA Module Control API for MSB.
...
/* RS-485 Channel Selection (Bit 3 of REG_MISC) */
typedef enum {
...
// Tests SPI bus integrity by writing and reading back test pattern (0x55AA)
FPGA_Status_t FPGA_Debug_Test_Echo(FPGA_HandleTypeDef *hbridge, uint16_t test_val, uint32_t timeout_ms);

// Updates physical status LEDs (VD35..VD37) via thread-safe RMW
...
// Triggers FPGA MultiBoot reconfiguration to Image 2 (Flash 1 MB offset)
FPGA_Status_t FPGA_Reboot_To_Image2(FPGA_HandleTypeDef *hbridge, uint32_t timeout_ms);

// Non-blocking running lights diagnostic sequence
...
// Configures Hard/Soft override masks and virtual simulation values
