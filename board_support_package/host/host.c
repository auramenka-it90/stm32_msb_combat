 ...
 *          over USART2 utilizing thread-safe DMA TX/RX, FSM parser,
...
volatile bool      is_bc_link_error     = true;
...
//-----------------------------------------------------------------------------
// Computes cumulative XOR checksum over byte buffer
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Byte-by-byte incoming packet FSM parser (BC -> MSB)
//-----------------------------------------------------------------------------
		...
		case RX_STATE_VERIFY_XOR:
			...
			if(expected_crc == received_crc){
				...
				if(expected_xor == received_xor){
					...
					// CRC & XOR Verified: Map incoming commands into fcs_commands
					BC_Data_t bc_data;
					...
					// Immediate asynchronous output write to FPGA
					fcs_set_sensors();
				} else {
					host_stats.rx_xor_err++;
				}
			} else {
				host_stats.rx_crc_err++;
			}
...
/* ========================================================================= */
/*  LOW-LEVEL DMA UART DRIVER                                                */
/* ========================================================================= */

//-----------------------------------------------------------------------------
// Initializes UART synchronization mutex and semaphore
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Arms DMA reception in ReceiveToIdle mode
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Sends raw byte buffer via DMA with thread-safety and OS task yield
//-----------------------------------------------------------------------------
bool	host_uart_send_raw(const uint8_t *data, uint16_t len){
	...
	// Wait for previous DMA TX completion (Yields task)
...
//-----------------------------------------------------------------------------
// Returns registered active UART handle
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Releases semaphore upon DMA TX completion (Called by central dispatcher)
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Processes received bytes from DMA buffer and restarts reception
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Hardware error recovery (Called by central dispatcher)
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Encodes FCS state into 32-byte packet and starts thread-safe DMA transmission
//-----------------------------------------------------------------------------
void	host_send_msb_packet(const FCS_State_t *state){
	if(!state || state->is_link_error){
		return;
	}
	...
	// 1. Target Distance: 1 LSB = 5m (Force 0 if SRD is inactive)
	if(state->srd){
	...
	// 2. Active Ammunition Type (4 bits)
	data.ammo_type = (uint8_t)state->ammo_type & 0x0FU;

	// 3. Flags Part 1 (Byte 3)
	...
	// 5. Build generic frame layout
	packet.start = MSB_START_BYTE_MSB; // 0xAA Header
	memcpy(packet.data, &data, MSB_DATA_SIZE);

	// 6. Calculate CRC-16-CCITT over Bytes 0 to 28 (Total 29 Bytes)
...
/* ========================================================================= */
/*  HAL RX EVENT CALLBACK                                                    */
/* ========================================================================= */

//-----------------------------------------------------------------------------
// HAL RX Event (Idle line detected or buffer filled)
//-----------------------------------------------------------------------------
